...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current = ExpressionWrapper(current_timezone.localize(datetime.datetime.now()) - F('get_date_time'), output_field=fields.DurationField())
        duration = ExpressionWrapper(F('top_date_time') - F('get_date_time'), output_field=fields.DurationField())
        remaining_reports = Report.objects.filter(
            Q(
                responsed=False,
                top_date_time__gte=current_timezone.localize(datetime.datetime.now()),
            )
